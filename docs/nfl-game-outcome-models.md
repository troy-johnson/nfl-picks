...
| market_rating_injury (spread power rating + injury load) | 65.0% | 0.22271 | 0.63657 | -0.00494 (0.00305) | 0.98 | 0.034 | Improves baseline and elo; behind elo_injury |
| elo_seed_injury (market-seeded Elo + injury load) | 65.0% | 0.22112 | 0.63280 | -0.00653 (0.00317) | 0.98 | 0.031 | Improves baseline and elo; behind elo_tuned_injury |
| elo_seed_market_rating_injury (seeded Elo + spread rating + injury load) | 65.3% | 0.22192 | 0.63483 | -0.00573 (0.00322) | 0.97 | 0.036 | Improves baseline and elo; behind elo_tuned_injury |
...
- Every logistic candidate without market input except elo_mov_injury, elo_mov2_injury, elo_tuned_injury, and market_rating_injury selected a 100% market weight in all 54 weeks, so its blended probability equals the market probability. Each of those four selected a 0.95 weight in 18 of 54 weeks (mean 0.98); their blends scored 0.21038, 0.21037, 0.21037, and 0.21035 Brier, each at most 0.00005 Brier (SE 0.00009) worse than the market's 0.21033. market_stack averaged a 0.78 market weight and its blend scored 0.21073 Brier, marginally worse than the market. Live picks are unchanged.
...
The market-prior round turns past closing spreads into team strength. The sign convention was verified first: a positive `spread_line` means the home team is favoured, and it agrees with the moneyline favourite on 99.5% of 4,362 games from 2010-2025 (the rest are line moves between the two captures). `spread_line` is populated for every nflverse season back to 1999, so no game lacks the input. The leakage rule is strict: only completed games whose `gameday` is strictly before the target game's `gameday` may contribute. The target game's own line, same-day lines, and every later line are excluded, and a test flips target-week and earlier-week spreads to prove the rule. For each game day the runner fits team ratings by weighted least squares to `spread_line = rating_home - rating_away + home_advantage`, over the trailing window of the current season's completed games at weight 1.0 plus the previous season's games at weight 0.5, regular and postseason, with the franchise aliases applied and a 0.1 ridge toward zero so the fit is identifiable. Neutral-site games drop the home-advantage column. The `market_rating_diff` feature is `rating_home - rating_away` without the home-advantage term.

The seeded Elo column `elo_diff_market_seed` runs the same walk as the tuned grid winner (aliases, signed margin factor, cap 24, K=16, home advantage 48, carryover 0.67), but at each season boundary it regresses each rating toward `1500 + 25 * market_rating` instead of 1500. One point of spread equals 25 Elo points under FiveThirtyEight's conversion. The market rating is fit once per season boundary from the previous season's full regular-plus-postseason closing spreads. The new `season_prior` parameter defaults to `None`, and with `None` the walk reproduces the plain walk bit-for-bit, so the earlier Elo columns and their tests are unchanged.

Three candidates use the new inputs: `market_rating_injury` (the spread rating plus the two injury features), `elo_seed_injury` (the seeded Elo plus the injury features), and `elo_seed_market_rating_injury` (both new inputs plus the injury features). None beats `elo_tuned_injury`. Pooled over 54 week blocks, Brier and log loss versus `elo_tuned_injury`: `market_rating_injury` is 0.00245 Brier (SE 0.00162) and 0.00547 log loss (SE 0.00371) worse; `elo_seed_injury` is 0.00086 Brier (SE 0.00061) and 0.00170 log loss (SE 0.00139) worse; `elo_seed_market_rating_injury` is 0.00166 Brier (SE 0.00132) and 0.00373 log loss (SE 0.00298) worse. All pooled differences are under two standard errors. Per season the pattern is the same for all three: slightly better in 2023 (each under one standard error), worse in 2024 and 2025. The largest deficit is `market_rating_injury` in 2024 at 0.00624 Brier (SE 0.00348), still under two standard errors. `elo_seed_injury` scored 65.0% accuracy, 0.22112 Brier, and 0.63280 log loss, the best of the round but behind `elo_tuned_injury`'s 0.22026 and 0.63110. Both new inputs do beat the plain `elo` candidate (0.00359 Brier for the seeded Elo, 0.00279 for the spread rating; SE 0.00205 and 0.00229), so the market prior carries real signal. But the spread rating correlates 0.82 with the signed-margin Elo difference on the 2023-2025 rows (0.91 with the seeded Elo), so on top of the injury features it adds nothing the tuned walk does not already capture. The two seeded candidates took a 100% market weight in all 54 weeks, so their blends equal the market (0.21033 Brier, 0.60807 log loss). `market_rating_injury` averaged a 0.98 market weight (0.95 in 18 of 54 weeks); its blend scored 0.21035 Brier and 0.60811 log loss, 0.00002 Brier (SE 0.00010) and 0.00004 log loss (SE 0.00023) worse than the recorded market. Nothing in this round changes live picks.
...
| market_rating_injury (spread power rating + injury load) | 0.23038 | 0.21094 | 0.22683 | 0.22271 |
| elo_seed_injury (market-seeded Elo + injury load) | 0.23228 | 0.20669 | 0.22441 | 0.22112 |
| elo_seed_market_rating_injury (seeded Elo + spread rating + injury load) | 0.23075 | 0.20966 | 0.22537 | 0.22192 |
